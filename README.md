...
raw source material. For videos and `--transcript-file` runs, the transcript is
saved to `./summaries/transcripts/<slug>.txt` (whisper-style timestamped lines,
re-usable with `--transcript-file`), and the summary gets a `**Transcript:**`
header linking it by relative path. For articles, the extracted body is saved as
a `…<slug>.source.md` sidecar beside the summary. Either way you get the summary
plus its reference material as a pair for import into a knowledge base.
