...
5. **`write_summary(summary, meta, out_dir)`** — Markdown file, slugified filename. Each `process_*` then also writes a raw sidecar sharing the summary's `YYYY-MM-DD_<slug>` stem (always on, no flag): `transcript_to_text(segments)` → `<stem>.transcript.txt` for video/transcript-file modes, the extracted article body → `<stem>.source.md` for articles. The transcript sidecar uses whisper-style `[M:SS --> M:SS]  text` lines, so it round-trips back through `--transcript-file`. `write_sidecar(text, summary_path, suffix)` does the write; `SummaryResult` carries `raw`/`raw_path`.
